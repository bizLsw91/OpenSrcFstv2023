...
const project_1 = {
    ...
    introDiv:
        <div><p>모던 자바스크립트 튜토리얼(<a href="#!" className="underline__blue">https://ko.javascript.info/</a>) 프로젝트에 기여해 보며 remote 저장소, origin, upstream이 무엇인지 제대로 이해해 보는 시간을 가질 예정입니다.</p> <p>master,  origin/master, upstream/master 이 세 가지가 무슨 차이가 있는지 구분을 못 하는 사람이라면 스프린트에 참여해 보세요. `git push -u origin branch` 명령어에서 `-u`가 어떤 역할을 하는지 궁금했던 분들도 환영합니다.</p>
        </div>,
